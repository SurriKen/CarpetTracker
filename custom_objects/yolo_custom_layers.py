...
from custom_objects.normalization_custom_layers import Mish


class YOLOResBlock(layers.Layer):
...
class YOLOv3ResBlock(layers.Layer):
...
class YOLOConvBlock(layers.Layer):
    """Unet block layer """
...
class DarkNetConvolutional(layers.Layer):
...
class DarkNetResBlock(layers.Layer):
...
class DarkNetUpsample(layers.Layer):

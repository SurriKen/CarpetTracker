...
class VideoClassifier:
    ...
    def load_model(self, weights: str = '') -> nn.Module:
        if weights and weights.split('.')[-1] == 'pt':
            ...
            # loading model with different parameters depending on CUDA availability
            if torch.cuda.is_available():
                ...
                self.model = torch.jit.load(weights, map_location=torch.device('cpu'))
                self.input_size = self.model.input_size
        else:
    ...
    def save_model(self, name, mode: str = 'last') -> None:
        ...
        model_scripted.save(os.path.join(ROOT_DIR, 'classification', name, f"{mode}.pt"))  # Save

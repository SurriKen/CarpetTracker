...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    tl = line_thickness or round(0.01 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
...
def add_headline_to_image(image, headline: str):
    if headline:
...
def get_obj_box_squares(clust_coords):
    ...
    keys = list(bbox.keys())
    for k in keys:
        if len(bbox[k]) < MIN_OBJ_SEQUENCE:
            bbox.pop(k)
    for k in bbox.keys():
        sq[k] = []
        for b in bbox[k]:

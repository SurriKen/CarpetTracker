    ...
    for camera in os.listdir(os.path.join(dataset, class_)):
        ...
        for video in os.listdir(os.path.join(dataset, class_, camera)):
            ...
            for k, v in boxes.items():
                boxes_norm[k] = [[c / shape[i % 2] for i, c in enumerate(coord)] for coord in v]
            box_data[class_][camera][video] = boxes_norm

save_data(data=box_data, folder_path=os.path.join(ROOT_DIR, 'tests'), filename=f'class_boxes_10_model2')
...
box_data = load_data(os.path.join(ROOT_DIR, 'tests/class_boxes_10_model2.dict'))
box_stat = {}
...
#             if class_stat[cl][c][k]['start']:
#                 # print(class_stat[cl][c][k])
#                 class_stat[cl][c][k]['start'] = get_stat_list(class_stat[cl][c][k]['start'])
#                 class_stat[cl][c][k]['end'] = get_stat_list(class_stat[cl][c][k]['end'])
#                 class_stat[cl][c][k]['min'] = get_stat_list(class_stat[cl][c][k]['min'])
#                 class_stat[cl][c][k]['max'] = get_stat_list(class_stat[cl][c][k]['max'])
#                 class_stat[cl][c][k]['mean'] = get_stat_list(class_stat[cl][c][k]['mean'])
#             else:
#                 class_stat[cl][c][k]['start'] = []
#                 class_stat[cl][c][k]['end'] = []
#                 class_stat[cl][c][k]['min'] = []
#                 class_stat[cl][c][k]['max'] = []
#                 class_stat[cl][c][k]['mean'] = []
#         print(c, k, 'start', class_stat[cl][c][k]['start'])
#         print(c, k, 'end', class_stat[cl][c][k]['end'])
#         print(c, k, 'min', class_stat[cl][c][k]['min'])
#         print(c, k, 'max', class_stat[cl][c][k]['max'])
#         print(c, k, 'mean', class_stat[cl][c][k]['mean'])
#     print()

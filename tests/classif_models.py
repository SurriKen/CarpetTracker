...
class Net(nn.Module):
    def __init__(self, device='cpu', num_classes: int = 5, frame_size=(256, 256)):
        ...
        self.dense = nn.Linear(16 * int(frame_size[0] / 4) * int(frame_size[1] / 4) * 10, num_classes, device=device)
...
class VideoClassifier:
    ...
    def load_model(self, weights: str = '') -> None:
        if weights and weights.split('.')[-1] == 'pt':
            ...
            self.model = Net(device=self.device, num_classes=self.num_classes, frame_size=self.frame_size)
    ...
    def get_x_batch(self, video_path: list) -> torch.Tensor:
        ...
        for vid in video_path:
            ...
            array = self.resize_video_array(array)
        ...
        # mean = torch.as_tensor(mean, dtype=x_train.dtype, device=self.torch_device)
        # std = torch.as_tensor(std, dtype=x_train.dtype, device=self.torch_device)
    ...
    def resize_video_array(self, array):
        new_array = []
        for a in array:
            im = Image.fromarray(a)
            im = im.resize(self.frame_size)

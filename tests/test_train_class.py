...
class Net(nn.Module):
    def __init__(self, device='cpu', num_classes: int = 5, input_size=(12, 256, 256, 3), concat_axis=2,
                 ...
                 dense_out: int = 256, dropout: float = 0.1):
    ...
    def forward(self, x):
        ...
        x = F.max_pool3d(F.relu(F.normalize(self.conv3d_1(F.dropout(x, self.dropout)))), (1, 2, 2))
        x = F.max_pool3d(F.relu(F.normalize(self.conv3d_2(F.dropout(x, self.dropout)))), (1, 2, 2))
        x = F.max_pool3d(F.relu(F.normalize(self.conv3d_3(F.dropout(x, self.dropout)))), (1, 2, 2))
        x = F.max_pool3d(F.relu(F.normalize(self.conv3d_4(F.dropout(x, self.dropout)))), (1, 2, 2))

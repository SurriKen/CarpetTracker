...
from utils import get_colors, add_headline_to_cv_image
...
def detect_synchro_video_polygon(
    ...
    result = dict(
        video_1=video_paths.get("model_1"),
        video_2=video_paths.get("model_2"),
        total_count=len(class_counter),
        class_aequence=class_counter,
        class_distribution=dict(Counter(class_counter))
    )
    return result
